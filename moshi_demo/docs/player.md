...
| `hour_period` | 2.0 | 时针一圈的秒数，也决定左键斩朝向变化快慢 |
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `tv_max_base` | 500 | 一管墨有多少 |
| `tv_regen_base` | 20 | 每秒回多少（只在不书写时回） |
...
`bullet_tv_drain × 秒数` 的流逝）。默认 500 ÷ 1.0 = 500 px；把 `tv_cost_per_px`
改成 0.5 就是 1000 px。写着写着墨见底时笔尖会「干涸」（`dry_pen`），线只画到买得起的位置。
